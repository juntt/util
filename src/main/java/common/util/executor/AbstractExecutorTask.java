...
public abstract class AbstractExecutorTask<U, V> {
	...
	protected boolean isAsync = true; // 执行模式 true: 异步; false: 同步
	// === 时间戳 ===
	...
	/** 执行超时/失败后异步重试策略 */
	public V async(U request) {
		return JExecutor.instance().execute(this, request);
	}
	...
	public String toString() {
		return String.format(
				"Task Name: %s, Status: %d, IsAsync: %s, StartTime: %d, EndTime: %d, Timeout Policy:(TimeoutSeconds: %d, TimeoutPolicy: %d), Retry Policy:(RetryCount: %d, RetryDelaySeconds: %d, RetryLogic: %d), RetriedCount: %d",
				name, status, isAsync, startTime, endTime, timeoutSeconds, timeoutPolicy, retryCount, retryDelaySeconds,
